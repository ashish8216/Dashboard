...
<form  class="form-horizontal" action="{{ route('page_images.update', $blog->id) }}" method="post" enctype="multipart/form-data" ><!-- form-horizontal Starts -->
  @method('PATCH') 
   @csrf
<div class="card-body">
...
<option value="{{$blog->page_id}}"> {{App\Models\Post_Categorie::where('id', $blog->page_id)->first()->name}}</option>
...
<option value="{{$blog->top_botton}}"> {{$blog->top_botton}} </option>
